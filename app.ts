...
let loadData = (): void => {
  ...
  let submitnewuserbtn: HTMLElement = document.getElementById("submitnewuser");
  ...
  (<any>Object).entries(elements).forEach(<T>(entry: T) => {
...
const editRow = (e: any): void => {
  ...
  editbtnlistener.removeEventListener("click", editRow);
...
const cancel = (e: any): void => {
  ...
  (<any>Object).entries(cancelVal).forEach(<T>(entry: T) => {
  ...
  (<any>Object).entries(removeHideClass).forEach(<T>(entry: T) => {
...
const save = (e: any): void => {
  ...
  document.getElementById(`edit_${index}`).addEventListener("click", editRow);
  // }
};
